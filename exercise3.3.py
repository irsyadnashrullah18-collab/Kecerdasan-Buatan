# Exercise 3.3 Python SVM Iris URL Classifications (Scatter Plot 2 Fitur Pertama)
from sklearn import datasets, svm
import pandas as pd
from matplotlib import pyplot
from pandas.plotting import scatter_matrix

url = 'https://gist.githubusercontent.com/curran/a08a1080b88344b0c8a7/raw/0e7a9b0a5d22642a06d3d5b9bcbad9890c8ee534/iris.csv'
df = pd.read_csv(url)

print(df.shape)
print(df.head(10))
print(df.tail(10))
print(df.describe())

# count NAN
print(df.isna().sum().sum())

# drop NAN values
df = df.dropna()
print(df.groupby('species').size())

# histograms
df.hist()
pyplot.show()

# scatter plot matrix
scatter_matrix(df)
pyplot.show()

X = df.values[:, :2]
s = df['species']
d = dict([(y, x) for x, y in enumerate(sorted(set(s)))])
y = [d[x] for x in s]

# --- MODIFIKASI EXERCISE 3.3: Scatter Plot 2 Fitur Pertama ---
pyplot.figure(figsize=(8, 6))
pyplot.scatter(X[:, 0], X[:, 1], c=y, cmap='viridis')
pyplot.xlabel('Sepal Length')
pyplot.ylabel('Sepal Width')
pyplot.title('Scatter Plot: Sepal Length vs Sepal Width')
pyplot.colorbar(label='Species Target (0, 1, 2)')
pyplot.show()
# -------------------------------------------------------------

clf = svm.SVC()
clf.fit(X, y)

# Predict the flower for a given Sepal length and width
p = clf.predict([[5.4, 3.2]])
print(p)